...
function getWorldIdConfig(signal: string): WorldIdConfig {
  return {
...
async function verifyWorldIdProof(proof: WorldIdProof, signal: string): Promise<VerificationResult> {
...
function mockVerifyWorldId(proof: WorldIdProof): VerificationResult {
...
function generateMockProof(): WorldIdProof {
...
async function processClaimWithWorldId(
  request: ClaimWithWorldIdRequest
...
async function verifyMultipleClaims(
  claims: ClaimWithWorldIdRequest[]
): Promise<Map<string, { success: boolean; txHash?: string; error?: string }>> {
  console.log(`[WORLDID] Batch verifying ${claims.length} claims...`);

  const results = new Map<string, { success: boolean; txHash?: string; error?: string }>();
...
export {
  ...
  verifyWorldIdProof,
  generateMockProof,
  processClaimWithWorldId,
  verifyMultipleClaims,
  getWorldIdConfig,
  mockVerifyWorldId,
};
