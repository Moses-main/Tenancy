...
const worldid_service_1 = require("./worldid-service");
const MOCK_PAYMENTS = [
    {
        propertyId: 0,
...
async function simulateCREWorkflow() {
    ...
    console.log('└────────────────────────────────────────────────────────────┘\n');
    await sleep(500);
    const riskMetrics = await (0, risk_service_1.simulateRiskScenario)('healthy');
    await sleep(400);
    console.log('\n┌────────────────────────────────────────────────────────────┐');
    ...
    for (let i = 0; i < results.length; i++) {
        if (results[i].success) {
            const proof = (0, worldid_service_1.generateMockProof)();
            const verification = await (0, worldid_service_1.verifyWorldIdProof)(proof, `0x742d35Cc6634C0532925a3b844Bc9e7595f0fEb${i}`);
