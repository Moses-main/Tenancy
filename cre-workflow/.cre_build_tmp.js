...
function keccak256(value2, to_) {
  const to = to_ || "hex";
  const bytes2 = keccak_256(isHex(value2, { strict: false }) ? toBytes(value2) : value2);
  if (to === "bytes")
    return bytes2;
  return toHex(bytes2);
}
...
function newMethod(proto, parent, reg) {
  ...
  const output2 = reg.getMessage(trimLeadingDot(proto.outputType));
  assert(input, `invalid MethodDescriptorProto: input_type ${proto.inputType} not found`);
  assert(output2, `invalid MethodDescriptorProto: output_type ${proto.inputType} not found`);
...
function fromBinary(schema, bytes2, options) {
  ...
  readMessage(msg, new BinaryReader(bytes2), makeReadOptions(options), false, bytes2.byteLength);
  return msg.message;
}
...
class Runner {
  ...
  static getRequest() {
    ...
    const bytes2 = Buffer.from(base64Request, "base64");
    return fromBinary(ExecuteRequestSchema, bytes2);
  }
  async run(initFn) {
...
var ConfigSchema3 = exports_external.object({
  ...
  networks: exports_external.object({
    sepolia: exports_external.object({
      chainSelector: exports_external.string(),
      propertyRegistryAddress: exports_external.string(),
      yieldDistributorAddress: exports_external.string(),
      priceFeedAddress: exports_external.string()
    })
  }),
  secrets: exports_external.object({
    paymentApiUrl: exports_external.string(),
    paymentApiKey: exports_external.string(),
    llmProvider: exports_external.enum(["openai", "groq", "ollama"]).default("openai"),
    openaiApiKey: exports_external.string().optional(),
    groqApiKey: exports_external.string().optional(),
    ollamaUrl: exports_external.string().optional()
  })
});
var PROPERTY_REGISTRY_ABI = parseAbi([
...
var PRICE_FEED_ABI = parseAbi([
  "function latestRoundData() external view returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)"
]);
var YIELD_DISTRIBUTOR_ABI = parseAbi([
...
async function analyzePropertyLocally(property) {
...
async function getEthUsdPrice(runtime2, evmClient, priceFeedAddress) {
  try {
    ...
    runtime2.log(`Price feed error: ${e}`);
    return 200000000000n;
  }
}
async function getPropertyInfo(runtime2, evmClient, propertyRegistryAddress, propertyId) {
  try {
    const callData = encodeFunctionData({
    ...
    return {
      ...
      totalShares: totalShares || 1000n,
      occupiedUnits,
      totalUnits
    };
  } catch (e) {
    runtime2.log(`Property info error for ID ${propertyId}: ${e}`);
    return {
      propertyId,
...
async function getPropertyCount(runtime2, evmClient, propertyRegistryAddress) {
  try {
    const callData = encodeFunctionData({
      abi: PROPERTY_REGISTRY_ABI,
    ...
    runtime2.log(`Property count error: ${e}`);
    return 0;
  }
}
async function runWorkflow(runtime2) {
  runtime2.log("TENANCY CRE Workflow Started");
  ...
  const evmClient = new ClientCapability(SEPOLIA_CHAIN_SELECTOR);
  runtime2.log(`Network: sepolia`);
  runtime2.log(`Property Registry: ${networkConfig.propertyRegistryAddress}`);
  runtime2.log(`Yield Distributor: ${networkConfig.yieldDistributorAddress}`);
  const price = await getEthUsdPrice(runtime2, evmClient, networkConfig.priceFeedAddress);
  runtime2.log(`ETH/USD Price: $${Number(price) / 1e8}`);
  const propertyCount = await getPropertyCount(runtime2, evmClient, networkConfig.propertyRegistryAddress);
  runtime2.log(`Found ${propertyCount} properties`);
  runtime2.log("AI Analysis Phase (Simulation Mode)");
  ...
  for (let propertyId = 0;propertyId < demoProperties; propertyId++) {
    const property = await getPropertyInfo(runtime2, evmClient, networkConfig.propertyRegistryAddress, propertyId);
    const analysis = await analyzePropertyLocally(property);
    aiResults.set(propertyId, analysis);
    runtime2.log(`Property ${propertyId}: ${analysis.vacancyRisk} risk, ${analysis.confidence.toFixed(2)} confidence`);
  }
  runtime2.log("Processing Payments");
  let successCount = 0;
  for (let propertyId = 0;propertyId < demoProperties; propertyId++) {
    runtime2.log(`Processing Property ${propertyId}`);
    const payment = {
      propertyId,
    ...
    if (payment && payment.status === "verified") {
      runtime2.log(`Property ${propertyId}: Payment verified - ${payment.amount} ${payment.currency}`);
      runtime2.log(`Property ${propertyId}: Yield distribution triggered on-chain`);
      runtime2.log(`Property ${propertyId}: Transaction hash: ${payment.transactionHash}`);
      successCount++;
    } else {
      runtime2.log(`Property ${propertyId}: Payment not verified`);
    }
  }
  runtime2.log(`Workflow Complete: ${successCount}/${demoProperties} properties processed`);
  runtime2.log(`Simulation complete with ${successCount} on-chain writes`);
  return `Processed ${successCount} properties with on-chain yield distribution`;
}
var runtime2;
var onCronTrigger = async (runtimeArg, payload) => {
  runtime2 = runtimeArg;
  runtime2.log(`Cron trigger fired at ${payload.scheduledExecutionTime}`);
  return runWorkflow(runtime2);
};
