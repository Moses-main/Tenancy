...
const RPC_URL = process.env.BASE_SEPOLIA_RPC_URL || 'https://base-sepolia.g.alchemy.com/v2/igSo1TQOzun0wSumQjuIM';
...
async function main() {
  ...
  try {
    const tx = await propertyRegistry.createAndListProperty(
      ...
      ethers.parseUnits("2500", 6),  // rentAmount (USDC/month)
      30,  // rentFrequency (days)
      ethers.parseUnits("1000", 18),  // initialSupply tokens
      "NYC Apartment Token",  // tokenName
      "NYCAT",  // tokenSymbol
      ethers.parseUnits("300000", 8),  // valuationUsd ($300k)
      ethers.parseUnits("500", 18),  // listingAmount (500 tokens for sale)
      ethers.parseUnits("100", 18)  // pricePerToken ($100)
    );
    ...
    if (listings.length > 0) {
      ...
      console.log(`Amount: ${ethers.formatUnits(lastListing.amount, 18)} tokens`);
      console.log(`Price per token: ${ethers.formatUnits(lastListing.pricePerToken, 18)} ETH`);
    }
  } catch (e) {
  ...
  try {
    ...
    if (listings.length > 0) {
      ...
      const totalCost = buyAmount * listing.pricePerToken / ethers.parseUnits("1", 18);
      console.log(`Cost for 10 tokens: ${ethers.formatUnits(totalCost, 6)} USDC`);
  ...
  try {
    ...
    if (properties.length > 0) {
      ...
      console.log(`Rent amount: ${ethers.formatUnits(property.rentAmount, 6)} USDC`);
      ...
      console.log(`\nTotal Yield Pool: ${ethers.formatUnits(yieldPool, 18)}`);
    }
  } catch (e) {
