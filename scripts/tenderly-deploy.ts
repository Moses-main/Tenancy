...
const TENDERLY_PROJECT = process.env.TENDERLY_PROJECT || "tenancy";
...
const NETWORKS: Record<string, TenderlyConfig> = {
  sepolia: {
    network: "sepolia",
    chainId: 11155111,
...
async function createVirtualTestnet(network: string): Promise<{ id: string; slug: string; rpcUrl: string }> {
  console.log(`Creating Tenderly Virtual TestNet for ${network}...`);
  ...
  if (!TENDERLY_API_KEY || !TENDERLY_ACCOUNT_ID) {
    console.log("Warning: TENDERLY_API_KEY or TENDERLY_ACCOUNT_ID not set");
    console.log("Creating local mock Virtual TestNet configuration...");
    return {
      id: "local-mock",
      slug: "tenancy-local",
  ...
  const response = await fetch(
    `https://api.tenderly.co/api/v1/account/${TENDERLY_ACCOUNT_ID}/project/${TENDERLY_PROJECT}/virtual-networks`,
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        name: `TENANCY-${network}-${Date.now()}`,
        description: "TENANCY Protocol Virtual TestNet for Chainlink Hackathon",
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create virtual testnet: ${error}`);
  }

  const data = await response.json() as any;
  const vtn = data.object;
  
  console.log(`Virtual TestNet created: ${vtn.id}`);
  console.log(`RPC URL: ${vtn.rpc_url}`);
...
async function deployToVirtualTestnet(vtnRpcUrl: string, privateKey: string, network: string): Promise<DeploymentResult> {
  console.log(`\nDeploying contracts to Virtual TestNet...`);
  ...
  const TENTokenFactory = await ethers.getContractFactory(
    `
  ...
  results.contracts.tenToken = await tenToken.getAddress();
  ...
  const PropertyRegistryFactory = await ethers.getContractFactory(
    ...
    `
  , wallet);
  const propertyRegistry = await PropertyRegistryFactory.deploy(wallet.address, priceFeed);
  await propertyRegistry.waitForDeployment();
  results.contracts.propertyRegistry = await propertyRegistry.getAddress();
  ...
  const YieldDistributorFactory = await ethers.getContractFactory(
    ...
    `
  , wallet);
  const yieldDistributor = await YieldDistributorFactory.deploy(wallet.address, priceFeed, ethers.ZeroAddress);
  await yieldDistributor.waitForDeployment();
  results.contracts.yieldDistributor = await yieldDistributor.getAddress();
  ...
  const PriceFeedConsumerFactory = await ethers.getContractFactory(
    ...
    `
  , wallet);
  const priceFeedConsumer = await PriceFeedConsumerFactory.deploy(priceFeed, ethers.ZeroAddress);
  await priceFeedConsumer.waitForDeployment();
  results.contracts.priceFeedConsumer = await priceFeedConsumer.getAddress();
  console.log(`   PriceFeedConsumer: ${results.contracts.priceFeedConsumer}`);

  console.log("\nConfiguring contracts...");
  
  const registry = await ethers.getContractAt(
    ["function setIssuer(address _issuer, bool _status) external"],
    results.contracts.propertyRegistry,
    wallet
  );
  await registry.setIssuer(wallet.address, true);
  console.log("   Issuer role granted");

  const tenTokenContract = await ethers.getContractAt(
    ["function setMinter(address _minter, bool _status) external"],
    results.contracts.tenToken,
    wallet
  );
  await tenTokenContract.setMinter(wallet.address, true);
...
async function main() {
  ...
  console.log("  TENANCY Protocol - Tenderly Virtual TestNet Deployment");
  console.log("=".repeat(60));
  console.log();
  ...
  if (!privateKey) {
    ...
    console.log("  TENDERLY_API_KEY=your_tenderly_api_key");
    console.log("  TENDERLY_ACCOUNT_ID=your_account_id");
    console.log("  TENDERLY_PROJECT=tenancy");
  ...
  console.log("Step 1: Creating Virtual TestNet...");
  const vtn = await createVirtualTestnet(network);
  console.log(`Virtual TestNet RPC: ${vtn.rpcUrl}`);
  console.log();

  console.log("Step 2: Deploying contracts to Virtual TestNet...");
  const results = await deployToVirtualTestnet(vtn.rpcUrl, privateKey, network);

  results.virtualTestnet.id = vtn.id;
  results.virtualTestnet.slug = vtn.slug;

  console.log("\n" + "=".repeat(60));
  console.log("Deployment Complete!");
  console.log("=".repeat(60));
  console.log();
  console.log("Virtual TestNet:");
  console.log(`  ID:     ${results.virtualTestnet.id}`);
  console.log(`  RPC:    ${results.virtualTestnet.rpcUrl}`);
  console.log(`  Chain:  ${results.virtualTestnet.chainId}`);
  console.log();
  ...
  console.log(`  PriceFeedConsumer: ${results.contracts.priceFeedConsumer}`);
  console.log();
  console.log(`Price Feed: ${results.priceFeed}`);

  const outputPath = path.join(process.cwd(), "tenderly-deployment.json");
  fs.writeFileSync(outputPath, JSON.stringify(results, null, 2));
  console.log(`\nResults saved to: ${outputPath}`);

  console.log("\nNext Steps:");
  console.log("1. Update .env with Virtual TestNet RPC:");
  console.log(`   TENDERLY_RPC_URL=${results.virtualTestnet.rpcUrl}`);
  console.log("\n2. Run tests on Virtual TestNet:");
  console.log(`   forge test --rpc-url ${results.virtualTestnet.rpcUrl}`);
  console.log("\n3. Open Tenderly Dashboard:");
  console.log(`   https://dashboard.tenderly.co/${TENDERLY_ACCOUNT_ID}/${TENDERLY_PROJECT}/virtual-networks`);
}
